...
#IMAGE_PATH = u'D:/SPB_Data/labelconv/dataset/test'
    ...
    for filename in files:
        ...
        if os.path.exists(image_path) :
            ...
            if detections['detection_scores'][0] > THRESH_HOLD :
                ...
                plate_str, plateTable,category_index_temp, CLASS_DIC,plate2line, class_index = plate_number_detect_fn(models,plate_new_img_np,ncat_index, class_index,result_path=result_path)
                ...
                if xfind == -1 :
                    recog_count += 1
                    if(plate_str in basefilename) :
                        true_recog_count += 1
                        if add_platename:
                            result_file = os.path.join(result_dir, basefilename + '_' + plate_str + ext)
                        else:
                            result_file = os.path.join(result_dir, basefilename  + ext)
                        ...
                        if add_platename:
                            ...
                            result_file = os.path.join(wrong_recog_dir, basefilename  + ext)
                        ...
                        makeJson(src_path=images_dir,image_filename=filename,dst_path=wrong_recog_dir,image_shape=image_np.shape, 
                             ...
                             plateNumber=plate_str,platebox = plate_box ,plateIndex = class_index,plate_shape = plate_new_img_np.shape, xratio=ratio, add_platenum=add_platename)
